...
//    @Value("${money.manager.frontend.url}")
    ...
    public void sendDailyIncomeExpenceRemainder(){
    ...
    public void sendDailyExpenseSummary() {
        ...
        List<ProfileEntity> profiles = profileRepository.findAll();
        for (ProfileEntity profile : profiles) {
            List<ExpenceDto> todaysExpenses = expenceService.getExpensesForUserOnDate(profile.getId(), LocalDate.now());
            if (!todaysExpenses.isEmpty()) {
                ...
                int i = 1;
                for(ExpenceDto expense : todaysExpenses) {
                    table.append("<tr>");
